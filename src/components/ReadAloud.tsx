...
export default function ReadAloud({ text }: ReadAloudProps) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-medium transition-all active:scale-95 ${
        speaking
          ? "bg-coral/10 text-coral dark:bg-coral/20"
          : "text-sub hover:text-coral hover:bg-sand/40 dark:hover:bg-dark-border"
      }`}
    >
      {speaking ? (
